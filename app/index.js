...
class Application {
	...
	async start(config) {
        ...
        app.locals.basePath = this.config.subfolder;
	...
	initAPI() {
	    ...
	    mainRouter.use('/console', consoleRouter);

	    app.use(`${this.config.subfolder}`, mainRouter);
	    app.use(`${this.config.subfolder}`, express.static(`${__dirname}/public`));
	}
	...
	initHelpers() {		
		...
		this.localUrl = `http://0.0.0.0:${this.config.port}${this.config.subfolder}/api/v1`;
